                    ...
                    <tr class="even:bg-white odd:bg-blue-50 border-b">
                        ...
                        <td class="px-6 py-4 space-x-3">
                            ...
                            <ion-icon name="trash-bin" title="delete" class="transition-all transform hover:scale-105 duration-300 w-6 h-6 text-red-600 cursor-pointer"
                                ...
                                onclick="$('#popup-modal form').attr('action', '{{ route('admin.work-category.destroy', $data->id) }}')"></ion-icon>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <div id="default-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-2xl max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <form action="{{ route('admin.work-category.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <!-- Modal body -->
    ...
    <script>
        ...
        <script>
            function btnAdd() {
                ...
                let url = "{{ route('admin.work-category.store') }}";
                $('#default-modal form').attr('action', url);
            }

            function btnEdit(id) {
                let url = "{{ route('admin.work-category.update', ':id') }}";
                url = url.replace(':id', id);
                $.ajax({
                    url: "{{ route('admin.work-category.show', ':id') }}".replace(':id', id),
                    type: "GET",
                    dataType: "JSON",
