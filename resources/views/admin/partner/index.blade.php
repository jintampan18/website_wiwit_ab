            ...
            <tbody>
                @foreach ($partners as $data)
                    <tr class="even:bg-white odd:bg-blue-50 border-b">
                        ...
                        <td class="px-6 py-4">
                            <img src="{{ asset('storage/partner/' . $data->logo) }}" alt=""
                        ...
                        <td class="px-6 py-4">
                            <div class="flex space-x-3">
                                ...
                                onclick="$('#popup-modal form').attr('action', '{{ route('admin.partner.destroy', $data->id) }}')"></ion-icon>
                            </div>
                        </td>
                    </tr>
                    @endforeach
    ...
    <script>
        ...
        <script>
            function btnAdd() {
                ...
                let url = "{{ route('admin.partner.store') }}";
                $('#default-modal form').attr('action', url);
            }

            function btnEdit(id) {
                let url = "{{ route('admin.partner.update', ':id') }}";
                url = url.replace(':id', id);
                $.ajax({
                    url: "{{ route('admin.partner.show', ':id') }}".replace(':id', id),
                    type: "GET",
                    dataType: "JSON",
