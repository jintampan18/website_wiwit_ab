            ...
            <tbody>
                ...
                <tr class="even:bg-white odd:bg-blue-50 border-b">
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                        <img src="{{ asset('storage/testimonial/' . $data->avatar) }}" alt=""
                    ...
                    <td class="px-6 py-4">
                        <div class="flex space-x-3">
                            ...
                            onclick="$('#popup-modal form').attr('action', '{{ route('admin.testimonial.destroy', $data->id) }}')"></ion-icon>
                        </div>
                    </td>
                </tr>
                @endforeach
                ...
                <form action="{{ route('admin.testimonial.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6 space-y-6">
                        <x-input id="name" type="text" label="Name" required name="name" value=""
                            placeholder="" class="" />
                        <x-input id="avatar" type="file" label="Avatar" required name="avatar" value=""
                            placeholder="" class="" />
                        <x-input id="job" type="text" label="Job" required name="job" value=""
                            placeholder="" class="" />
                        <x-input id="position" type="text" label="Position" required name="position"
                            value="" placeholder="" class="" />
                        <x-textarea id="testimonial" label="Testimonial" required name="testimonial" value=""
                            placeholder="" class="" />
                    </div>
                    <!-- Modal footer -->
    ...
    <script>
        ...
        <script>
            ...
            function btnEdit(id) {
                ...
                $.ajax({
                    url: "{{ route('admin.testimonial.show', ':id') }}".replace(':id', id),
                    type: "GET",
                    dataType: "JSON",
                    success: function(data) {
                        ...
                        $('#default-modal form #job').val(data.job);
                        $('#default-modal form #position').val(data.position);
                        $('#default-modal form #testimonial').val(data.testimonial);
                        $('#default-modal form #avatar').after(
                            `<a href="{{ asset('storage/testimonial/${data.avatar}') }}" target="_blank" class="text-blue-500">Lihat avatar</a>`
                        );
                    },
                    error: function() {
                        Swal.fire(
