<x-app-layout>
    ...
    <div class="relative overflow-x-auto rounded-lg custom-box-shadow  overflow-y-auto">
        ...
        <div class="overflow-x-auto">
            ...
            <tbody>
                @foreach ($consultationRequestsPaginated as $data)
                    <tr class="even:bg-white odd:bg-blue-50 border-b">
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                            {{ $data->name }}
                        </th>
                        <td class="px-6 py-4 space-x-3 min-w-max">
                            <div class="w-[150px]">
                                {{ $data->consultationRequestCategory->name }}
                            </div>
                        </th>
                        ...
                        <td class="px-6 py-4 flex items-center min-h-max">
                            <div class="flex justify-center items-center space-x-3 h-[100px]">
                                ...
                                onclick="btnShow('{{ $data->id }}')"></ion-icon>
                                ...
                                onclick="$('#popup-modal form').attr('action', '{{ route('admin.consultation-request.destroy', $data->id) }}')"></ion-icon>
                            </div>
                        </td>
                    </tr>
                    @endforeach
    ...
    <div class="pt-4">
        {{ $consultationRequestsPaginated->links() }}
    </div>
    ...
    <div id="default-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-2xl max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <form action="{{ route('admin.consultation-request.store') }}" method="POST"
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <x-input id="phone_number" type="text" label="Phone Number" required name="phone_number"
                            value="" placeholder="" class="" />
                        <x-input id="email" type="text" label="Email" required name="email" value=""
                            placeholder="" class="" />
                        <x-input id="subject" type="text" label="Subject" required name="subject"
                            value="" placeholder="" class="" />
                        <x-textarea id="message" type="text" label="Message" required name="message"
                            value="" placeholder="" class="" />
                    </div>
                </form>
            </div>
    ...
    <script>
        ...
        <script>
            function btnShow(id) {
                let url = "{{ route('admin.consultation-request.update', ':id') }}";
                url = url.replace(':id', id);
                $.ajax({
                    url: "{{ route('admin.consultation-request.show', ':id') }}".replace(':id', id),
                    type: "GET",
                    dataType: "JSON",
                    success: function(data) {
                        ...
                        $('#default-modal form #phone_number').val(data.phone_number);
                        $('#default-modal form #email').val(data.email);
                        $('#default-modal form #subject').val(data.subject);
