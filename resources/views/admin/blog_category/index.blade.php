                    ...
                    <tr class="even:bg-white odd:bg-blue-50 border-b">
                        ...
                        <td class="px-6 py-4 space-x-3">
                            ...
                            <ion-icon name="trash-bin" title="delete" class="transition-all transform hover:scale-105 duration-300 w-6 h-6 text-red-600 cursor-pointer"
                                ...
                                onclick="$('#popup-modal form').attr('action', '{{ route('admin.blog-category.destroy', $data->id) }}')"></ion-icon>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <script>
        ...
        <script>
            function btnAdd() {
                ...
                let url = "{{ route('admin.blog-category.store') }}";
                $('#default-modal form').attr('action', url);
            }

            function btnEdit(id) {
                let url = "{{ route('admin.blog-category.update', ':id') }}";
                url = url.replace(':id', id);
                $.ajax({
                    url: "{{ route('admin.blog-category.show', ':id') }}".replace(':id', id),
                    type: "GET",
                    dataType: "JSON",
