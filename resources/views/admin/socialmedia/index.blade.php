            ...
            <tbody>
                @foreach ($socialMedias as $data)
                    <tr class="even:bg-white odd:bg-blue-50 border-b">
                        ...
                        <td class="px-6 py-4">
                            <ion-icon name="{{ $data->icon }}" class="w-6 h-6 text-gray-500"></ion-icon>
                        </td>
                        <td class="px-6 py-4 min-w-max">
                            <div class="w-[400px]">
                                <a href="{{ $data->url }}" target="_blank" class="text-blue-500">{{ $data->url }}</a>
                            </div>
                        </td>
                        <td class="px-6 py-4">
                            <div class="space-x-3 flex">
                                ...
                                onclick="$('#popup-modal form').attr('action', '{{ route('admin.social-media.destroy', $data->id) }}')"></ion-icon>
                            </div>
                        </td>
                    </tr>
                    @endforeach
                ...
                <form action="{{ route('admin.social-media.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6 space-y-6">
                        <x-input id="name" type="text" label="Name" required name="name" value=""
                            placeholder="" class="" />
                        <x-input id="icon" type="text" label="Icon" required name="icon" value=""
                            placeholder="" class="" />
                        <x-input id="url" type="text" label="Url" required name="url" value=""
                            placeholder="" class="" />
                    </div>
                    <!-- Modal footer -->
    ...
    <script>
        ...
        <script>
            function btnAdd() {
                ...
                let url = "{{ route('admin.social-media.store') }}";
                $('#default-modal form').attr('action', url);
            }

            function btnEdit(id) {
                let url = "{{ route('admin.social-media.update', ':id') }}";
                url = url.replace(':id', id);
                $.ajax({
                    url: "{{ route('admin.social-media.show', ':id') }}".replace(':id', id),
                    type: "GET",
                    dataType: "JSON",
                    success: function(data) {
                        ...
                        $('#default-modal form #icon').val(data.icon);
                        $('#default-modal form #url').val(data.url);
                    },
                    error: function() {
                        Swal.fire(
                            'Error!',
