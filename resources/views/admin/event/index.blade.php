            ...
            <tbody>
                @foreach ($events as $data)
                    <tr class="even:bg-white odd:bg-blue-50 border-b">
                        ...
                        <td class="px-6 py-4 min-w-max">
                            <div class="w-[100px]">
                                <a href="{{ asset('storage/event/' . $data->thumbnail) }}" target="_blank"
                                    class="text-blue-500">See thumbnail</a>
                                </div>
                        </td>
                        <td class="px-6 py-4 min-w-max">
                            <div class="flex space-x-3 w-[100px] justify-center">
                                ...
                                onclick="$('#popup-modal form').attr('action', '{{ route('admin.event.destroy', $data->id) }}')"></ion-icon>
                            </div>
                            </td>
                        </tr>
                        @endforeach
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <form action="{{ route('admin.event.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="p-6 space-y-6">
                        <x-select id="event_category_id" label="Material Category" required name="event_category_id">
                        ...
                        <x-input id="name" type="text" label="Name" required name="name" value=""
                            placeholder="" class="" />
                        <x-input id="location" type="text" label="Location" required name="location" value=""
                            placeholder="" class="" />
    ...
    <script>
        ...
        <script>
            function btnAdd() {
                ...
                let url = "{{ route('admin.event.store') }}";
            ...
            function btnEdit(id) {
                let url = "{{ route('admin.event.update', ':id') }}";
                url = url.replace(':id', id);
                $.ajax({
                    url: "{{ route('admin.event.show', ':id') }}".replace(':id', id),
                    type: "GET",
                    dataType: "JSON",
                    success: function(data) {
                        ...
                        $('#default-modal form #event_category_id').val(data.event_category_id);
                        ...
                        $("#default-modal form #date").val(formattedDate);
                        $('#default-modal form #thumbnail').after(
                            `<a href="{{ asset('storage/event/${data.thumbnail}') }}" target="_blank" class="text-blue-500">See thumbnail</a>`
                        );
                    },
                    error: function() {
                        Swal.fire(
