                    ...
                    <tr class="even:bg-white odd:bg-blue-50 border-b">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex text-center space-x-3">
                                ...
                                onclick="$('#popup-modal form').attr('action', '{{ route('admin.material-category.destroy', $data->id) }}')"></ion-icon>
                            </div>
                        </td>
                    </tr>
                @endforeach
    ...
    <div id="default-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative w-full max-w-2xl max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow">
                ...
                <form action="{{ route('admin.material-category.store') }}" method="POST"
    ...
    <script>
        ...
        <script>
            function btnAdd() {
                ...
                let url = "{{ route('admin.material-category.store') }}";
                $('#default-modal form').attr('action', url);
            }

            function btnEdit(id) {
                let url = "{{ route('admin.material-category.update', ':id') }}";
                url = url.replace(':id', id);
                $.ajax({
                    url: "{{ route('admin.material-category.show', ':id') }}".replace(':id', id),
                    type: "GET",
                    dataType: "JSON",
